...
for epoch in range(1):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        Train_Accu.append(train_correct /32)
        Train_Cost.append(train_loss)
fig,(ax1, ax2) = plt.subplots(2, sharex=True, figsize=(12,7))
ax1.plot(Train_Cost,label='Train Cost',c='red')
ax1.set_title('Train Cost')
ax1.grid(True)
ax1.legend(loc=0)
ax2.set_title('Accuacy')
ax2.plot(Train_Accu,label='Accuracy',c='green')
ax2.grid(True)
plt.legend(loc=0)
plt.savefig('/content/figure1.jpg')
plt.show()
    # evaluation--------------------------------
model.eval()
eval_loss = 0.
...
for batch_x, batch_y in test_loader:
    ...
    Test_Accu.append(eval_acc )
    Test_Cost.append(eval_loss )
